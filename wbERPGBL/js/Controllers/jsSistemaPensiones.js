...
function establecerData(data) {
    $('#txtSistema').val(data.sistema_pension);
        ...
        if (jsonData.result == 'error') {
            ...
            if (jsonData.body != null) {
                $.each(jsonData.body, function (index, item) {
                    ...
                    bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idsistema_pension) + '</td>';
                    bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idsistema_pension + '"></span></td>';
                    bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idsistema_pension + '"></span></td></tr>';
                    $('#tbl_body').append(bodyTable);
                    bodyTable = '';
                    $('#modificar' + item.idsistema_pension).unbind();
                    $('#modificar' + item.idsistema_pension).on('click', function () {
                        establecerData(item);
                        ...
                        $('#btnGuardar').on('click', function () {
                            ...
                            $('#frmInsertar').submit(function (e) {
                                e.preventDefault();
                                modificar(item.idsistema_pension);
                            });
                        });
                    });
                    $('#eliminar' + item.idsistema_pension).unbind();
                    $('#eliminar' + item.idsistema_pension).on('click', function () {
                        $('#btn_mensaje_aceptar').attr('disabled', false);
                        $('#btn_mensaje_cancelar').attr('disabled', false);
                        $('#modal_titulo_mensaje').html('Advertencia');
                        $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.sistema_pension + '</b> permanentemente?');
                        ...
                        $('#btn_mensaje_aceptar').on('click', function () {
                            eliminar(item.idsistema_pension);
                        });
                    });
                    $('#estado' + item.idsistema_pension).unbind();
                    $('#estado' + item.idsistema_pension).on('click', function () {
                        $('#btn_mensaje_aceptar').attr('disabled', false);
                        $('#btn_mensaje_cancelar').attr('disabled', false);
                        $('#modal_titulo_mensaje').html('Advertencia');
                        $('#cuerpo_mensaje').html('¿Está seguro que desea cambiar el estado de <b>' + item.sistema_pension + '</b>?');
                        $('#modal_mensaje').modal('show');
                        $('#btn_mensaje_aceptar').unbind();
                        $('#btn_mensaje_aceptar').html('<span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Modificar');
                        $('#btn_mensaje_aceptar').on('click', function () {
                            modificar_estado(item.idsistema_pension, Math.abs(item.estado - 1));
                        });
                    });
...
function modificar(id) {
    var query = 'IDSISTEMA_PENSION=' + id +
        '&SISTEMA_PENSION="' + encodeURIComponent($('#txtSistema').val()) + '"' +
        '&CODIGO="' + encodeURIComponent($('#txtCodigo').val()) + '"';

    $.ajax({
        url: 'sistema_pensiones_frmMantenimientoSistemaPensiones.aspx/modificar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function insertar() {
    var query = 'SISTEMA_PENSION="' + encodeURIComponent($('#txtSistema').val()) + '"' +
    '&CODIGO="' + encodeURIComponent($('#txtCodigo').val()) + '"';

    $.ajax({
        url: 'sistema_pensiones_frmMantenimientoSistemaPensiones.aspx/insertar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function eliminar(id) {
    var query = 'IDSISTEMA_PENSION=' + id;
    $.ajax({
        url: 'sistema_pensiones_frmMantenimientoSistemaPensiones.aspx/eliminar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function modificar_estado(id, estado) {
    var query = 'IDSISTEMA_PENSION=' + id +
       '&ESTADO=' + estado;;
    $.ajax({
        url: 'sistema_pensiones_frmMantenimientoSistemaPensiones.aspx/modificar_estado',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
