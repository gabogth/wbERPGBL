...
function establecerData(data) {
    $('#txtTipoCuenta').val(data.tipo_cuenta);
...
function buscar(indexPag, cantidad) {
    $.ajax({
        url: 'tipoCuenta_frmMantenimientoTipoCuenta.aspx/buscar',
        type: "POST",
        dataType: 'json',
...
function showTableHTML(data, indexPag, cantidad) {
    ...
    $.each(data, function (index, item) {
        ...
        bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idtipo_cuenta_contable) + '</td>';
        bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idtipo_cuenta_contable + '"></span></td>';
        bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idtipo_cuenta_contable + '"></span></td></tr>';
        $('#tbl_body').append(bodyTable);
        bodyTable = '';
        $('#modificar' + item.idtipo_cuenta_contable).unbind();
        $('#modificar' + item.idtipo_cuenta_contable).on('click', function () {
            establecerData(item);
            $('#modal_insertar_titulo').html('MODIFICAR ' + item.tipo_cuenta);
            $('#modalInsertar').modal('show');
            $('#btnGuardar').attr('disabled', false);
            $('#btnCancelar').attr('disabled', false);
            $('#btnGuardar').unbind();
            ...
            $('#btnGuardar').on('click', function () {
                ...
                $('#frmInsertar').submit(function (e) {
                    e.preventDefault();
                    modificar(item.idtipo_cuenta_contable);
                });
            });
        });
        $('#eliminar' + item.idtipo_cuenta_contable).unbind();
        $('#eliminar' + item.idtipo_cuenta_contable).on('click', function () {
            $('#btn_mensaje_aceptar').attr('disabled', false);
            $('#btn_mensaje_cancelar').attr('disabled', false);
            $('#modal_titulo_mensaje').html('Advertencia');
            $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.tipo_cuenta + '</b> permanentemente?');
            ...
            $('#btn_mensaje_aceptar').on('click', function () {
                eliminar(item.idtipo_cuenta_contable);
            });
        });
        $('#estado' + item.idtipo_cuenta_contable).unbind();
        $('#estado' + item.idtipo_cuenta_contable).on('click', function () {
            $('#btn_mensaje_aceptar').attr('disabled', false);
            $('#btn_mensaje_cancelar').attr('disabled', false);
            $('#modal_titulo_mensaje').html('Advertencia');
            $('#cuerpo_mensaje').html('¿Está seguro que desea modificar el estado de <b>' + item.tipo_cuenta + '</b> permanentemente?');
            $('#modal_mensaje').modal('show');
            $('#btn_mensaje_aceptar').unbind();
            $('#btn_mensaje_aceptar').html('<span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Modificar');
            $('#btn_mensaje_aceptar').on('click', function () {
                modificar_estado(item.idtipo_cuenta_contable, Math.abs(item.estado - 1));
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'tipoCuenta_frmMantenimientoTipoCuenta.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(id) {
    ...
    formData["ID"] = id;
    $.ajax({
        url: 'tipoCuenta_frmMantenimientoTipoCuenta.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData),
...
function modificar_estado(id, estado) {
    $.ajax({
        url: 'tipoCuenta_frmMantenimientoTipoCuenta.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(id) {
    $.ajax({
        url: 'tipoCuenta_frmMantenimientoTipoCuenta.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + id + '}',
