    ...
    $.ajax({
        ...
        if (jsonData.result == 'error') {
            ...
            $.each(jsonData.body, function (index, item) {
                ...
                bodyTable += '<td>' + item.moneda + '</td>';
                bodyTable += '<td>' + item.simbolo + '</td>';
                bodyTable += '<td>' + item.codigo + '</td>';
                bodyTable += '<td>' + (item.local == 1 ? 'SI' : 'NO') + '</td>';
                bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idmoneda) + '</td>';
                bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idmoneda + '"></span></td>';
                bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idmoneda + '"></span></td></tr>';
                $('#tbl_body').append(bodyTable);
                bodyTable = '';
                $('#modificar' + item.idmoneda).unbind();
                $('#modificar' + item.idmoneda).on('click', function () {
                    establecerData(item);
                    ...
                    $('#btnGuardar').on('click', function () {
                        ...
                        $('#frmInsertar').submit(function (e) {
                            e.preventDefault();
                            modificar(item.idmoneda);
                        });
                    });
                });
                $('#eliminar' + item.idmoneda).unbind();
                $('#eliminar' + item.idmoneda).on('click', function () {
                    $('#btn_mensaje_aceptar').attr('disabled', false);
                    $('#btn_mensaje_cancelar').attr('disabled', false);
                    $('#modal_titulo_mensaje').html('Advertencia');
                    $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.moneda + '</b> permanentemente?');
                    ...
                    $('#btn_mensaje_aceptar').on('click', function () {
                        eliminar(item.idmoneda);
                    });
                });
                $('#estado' + item.idmoneda).unbind();
                $('#estado' + item.idmoneda).on('click', function () {
                    $('#btn_mensaje_aceptar').attr('disabled', false);
                    $('#btn_mensaje_cancelar').attr('disabled', false);
                    $('#modal_titulo_mensaje').html('Advertencia');
                    ...
                    $('#btn_mensaje_aceptar').on('click', function () {
                        modificar_estado(item.idmoneda, Math.abs(item.estado - 1));
                    });
                });
...
function insertar() {
    ...
    $.ajax({
        url: 'moneda_frmMantenimientoMoneda.aspx/insertar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function eliminar(id) {
    var query = 'IDMONEDA=' + id;
    $.ajax({
        url: 'moneda_frmMantenimientoMoneda.aspx/eliminar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
