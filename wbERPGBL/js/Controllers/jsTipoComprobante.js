...
function establecerData(data) {
    $('#txtTipoComprobante').val(data.tipo_comprobante);
...
function buscar(indexPag, cantidad) {
    ...
    $.ajax({
        url: 'tipocomprobante_frmMantenimientoTipoComprobante.aspx/buscar',
        type: "GET",
        dataType: 'json',
        ...
        if (jsonData.result == 'error') {
            ...
            $.each(jsonData.body, function (index, item) {
                ...
                bodyTable += '<td>' + item.tipo_comprobante + '</td>';
                bodyTable += '<td>' + item.factor + '</td>';
                bodyTable += '<td>' + item.porcentaje + '</td>';
                bodyTable += '<td>' + item.codigo + '</td>';
                bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idtipo_comprobante) + '</td>';
                bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idtipo_comprobante + '"></span></td>';
                bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idtipo_comprobante + '"></span></td></tr>';
                $('#tbl_body').append(bodyTable);
                bodyTable = '';
                $('#modificar' + item.idtipo_comprobante).unbind();
                $('#modificar' + item.idtipo_comprobante).on('click', function () {
                    establecerData(item);
                    ...
                    $('#btnGuardar').on('click', function () {
                        ...
                        $('#frmInsertar').submit(function (e) {
                            e.preventDefault();
                            modificar(item.idtipo_comprobante);
                        });
                    });
                });
                $('#eliminar' + item.idtipo_comprobante).unbind();
                $('#eliminar' + item.idtipo_comprobante).on('click', function () {
                    $('#btn_mensaje_aceptar').attr('disabled', false);
                    $('#btn_mensaje_cancelar').attr('disabled', false);
                    $('#modal_titulo_mensaje').html('Advertencia');
                    $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.tipo_comprobante + '</b> permanentemente?');
                    ...
                    $('#btn_mensaje_aceptar').on('click', function () {
                        eliminar(item.idtipo_comprobante);
                    });
                });
                $('#estado' + item.idtipo_comprobante).unbind();
                $('#estado' + item.idtipo_comprobante).on('click', function () {
                    $('#btn_mensaje_aceptar').attr('disabled', false);
                    $('#btn_mensaje_cancelar').attr('disabled', false);
                    $('#modal_titulo_mensaje').html('Advertencia');
                    $('#cuerpo_mensaje').html('¿Está seguro que desea cambiar el estado de <b>' + item.tipo_comprobante + '</b>?');
                    $('#modal_mensaje').modal('show');
                    $('#btn_mensaje_aceptar').unbind();
                    $('#btn_mensaje_aceptar').html('<span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Modificar');
                    $('#btn_mensaje_aceptar').on('click', function () {
                        modificar_estado(item.idtipo_comprobante, Math.abs(item.estado - 1));
                    });
                });
...
function modificar(id) {
    var query = 'IDTIPO_COMPROBANTE=' + id +
        '&CODIGO="' + encodeURIComponent($('#txtCodigo').val()) + '"' +
        '&TIPO_COMPROBANTE="' + encodeURIComponent($('#txtTipoComprobante').val()) + '"' +
        '&FACTOR=' + encodeURIComponent($('#txtFactor').val()) +
        '&PORCENTAJE="' + encodeURIComponent($('#txtPorcentaje').val()) + '"';

    $.ajax({
        url: 'tipocomprobante_frmMantenimientoTipoComprobante.aspx/modificar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function insertar() {
    ...
    $.ajax({
        url: 'tipocomprobante_frmMantenimientoTipoComprobante.aspx/insertar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function eliminar(id) {
    var query = 'IDTIPO_COMPROBANTE=' + id;
    $.ajax({
        url: 'tipocomprobante_frmMantenimientoTipoComprobante.aspx/eliminar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function modificar_estado(id, estado) {
    var query = 'IDTIPO_COMPROBANTE=' + id +
       '&ESTADO=' + estado;;
    $.ajax({
        url: 'tipocomprobante_frmMantenimientoTipoComprobante.aspx/modificar_estado',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
