...
function establecerData(data) {
    $('#txtPuestoTrabajador').val(data.puesto_trabajador);
}
...
function buscar(indexPag, cantidad) {
    ...
    $.ajax({
        ...
        data: query,
        beforeSend: function () {
            $('#tbl_body').html('<tr><td colspan="5" class="text-center"><span class="fa fa-hourglass faa-slow faa-spin animated"></span>&nbsp;&nbsp;&nbsp;Procesando...</td></tr>');
        }
    }).done(function (data) {
        ...
        if (jsonData.result == 'error') {
            ...
            $.each(jsonData.body, function (index, item) {
                ...
                bodyTable += '<td>' + item.puesto_trabajador + '</td>';
                bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idpuesto_trabajador) + '</td>';
                bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idpuesto_trabajador + '"></span></td>';
                bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idpuesto_trabajador + '"></span></td></tr>';
                $('#tbl_body').append(bodyTable);
                bodyTable = '';
                $('#modificar' + item.idpuesto_trabajador).unbind();
                $('#modificar' + item.idpuesto_trabajador).on('click', function () {
                    establecerData(item);
                    ...
                    $('#btnGuardar').on('click', function () {
                        ...
                        $('#frmInsertar').submit(function (e) {
                            e.preventDefault();
                            modificar(item.idpuesto_trabajador);
                        });
                    });
                });
                $('#eliminar' + item.idpuesto_trabajador).unbind();
                $('#eliminar' + item.idpuesto_trabajador).on('click', function () {
                    $('#btn_mensaje_aceptar').attr('disabled', false);
                    $('#btn_mensaje_cancelar').attr('disabled', false);
                    $('#modal_titulo_mensaje').html('Advertencia');
                    $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.puesto_trabajador + '</b> permanentemente?');
                    ...
                    $('#btn_mensaje_aceptar').on('click', function () {
                        eliminar(item.idpuesto_trabajador);
                    });
                });
...
function modificar(id) {
    var query = 'IDPUESTO_TRABAJADOR=' + id +
        '&PUESTO_TRABAJADOR="' + encodeURIComponent($('#txtPuestoTrabajador').val()) + '"';

    $.ajax({
        url: 'puestotrabajador_frmMantenimientoPuestoTrabajador.aspx/modificar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function insertar() {
    var query = 'PUESTO_TRABAJADOR="' + encodeURIComponent($('#txtPuestoTrabajador').val()) + '"';

    $.ajax({
        url: 'puestotrabajador_frmMantenimientoPuestoTrabajador.aspx/insertar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function eliminar(id) {
    var query = 'IDPUESTO_TRABAJADOR=' + id;
    $.ajax({
        url: 'puestotrabajador_frmMantenimientoPuestoTrabajador.aspx/eliminar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
