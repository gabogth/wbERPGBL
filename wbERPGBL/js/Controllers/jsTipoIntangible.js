...
function establecerData(data) {
    $('#txtTipoIntangible').val(data.tipo_intangible);
...
function buscar(indexPag, cantidad) {
    $.ajax({
        url: 'tipoIntangibleMantenimiento.aspx/buscar',
        type: "POST",
        dataType: 'json',
...
function showTableHTML(data, indexPag, cantidad) {
    ...
    $.each(data, function (index, item) {
        ...
        bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idtipo_intangible) + '</td>';
        bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idtipo_intangible + '"></span></td>';
        bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idtipo_intangible + '"></span></td></tr>';
        $('#tbl_body').append(bodyTable);
        bodyTable = '';
        $('#modificar' + item.idtipo_intangible).unbind();
        $('#modificar' + item.idtipo_intangible).on('click', function () {
            establecerData(item);
            $('#modal_insertar_titulo').html('MODIFICAR ' + item.tipo_intangible);
            $('#modalInsertar').modal('show');
            $('#btnGuardar').attr('disabled', false);
            $('#btnCancelar').attr('disabled', false);
            $('#btnGuardar').unbind();
            ...
            $('#btnGuardar').on('click', function () {
                ...
                $('#frmInsertar').submit(function (e) {
                    e.preventDefault();
                    modificar(item.idtipo_intangible);
                });
            });
        });
        $('#eliminar' + item.idtipo_intangible).unbind();
        $('#eliminar' + item.idtipo_intangible).on('click', function () {
            $('#btn_mensaje_aceptar').attr('disabled', false);
            $('#btn_mensaje_cancelar').attr('disabled', false);
            $('#modal_titulo_mensaje').html('Advertencia');
            $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.tipo_intangible + '</b> permanentemente?');
            ...
            $('#btn_mensaje_aceptar').on('click', function () {
                eliminar(item.idtipo_intangible);
            });
        });
        $('#estado' + item.idtipo_intangible).unbind();
        $('#estado' + item.idtipo_intangible).on('click', function () {
            $('#btn_mensaje_aceptar').attr('disabled', false);
            $('#btn_mensaje_cancelar').attr('disabled', false);
            $('#modal_titulo_mensaje').html('Advertencia');
            $('#cuerpo_mensaje').html('¿Está seguro que desea modificar el estado de <b>' + item.tipo_intangible + '</b> permanentemente?');
            $('#modal_mensaje').modal('show');
            $('#btn_mensaje_aceptar').unbind();
            $('#btn_mensaje_aceptar').html('<span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Modificar');
            $('#btn_mensaje_aceptar').on('click', function () {
                modificar_estado(item.idtipo_intangible, Math.abs(item.estado - 1));
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'tipoIntangibleMantenimiento.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(id) {
    ...
    formData["ID"] = id;
    $.ajax({
        url: 'tipoIntangibleMantenimiento.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData),
...
function modificar_estado(id, estado) {
    $.ajax({
        url: 'tipoIntangibleMantenimiento.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(id) {
    $.ajax({
        url: 'tipoIntangibleMantenimiento.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + id + '}',
