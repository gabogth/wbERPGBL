...
function establecerData(data) {
    ...
    $('#txtCaracteristicas').val(data.caracteristicas);
    setSelect2('#cbTipoExistencia', data.idtipo_existencia, data.tipo_existencia);
}
...
function buscar(indexPag, cantidad) {
    $.ajax({
        url: 'productoMantenimiento.aspx/buscar',
        type: "POST",
        dataType: 'json',
...
function showTableHTML(data, indexPag, cantidad) {
    ...
    $.each(data, function (index, item) {
        ...
        bodyTable += '<td>' + item.producto + '</td>';
        bodyTable += '<td title="' + item.caracteristicas + '">' + (item.caracteristicas.length > 20 ? item.caracteristicas.substring(0, 19) : item.caracteristicas) + '</td>';
        bodyTable += '<td>' + item.tipo_existencia + '</td>';
        bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idproducto) + '</td>';
        bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idproducto + '"></span></td>';
        bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idproducto + '"></span></td></tr>';
        $('#tbl_body').append(bodyTable);
        bodyTable = '';
        $('#modificar' + item.idproducto).unbind();
        $('#modificar' + item.idproducto).on('click', function () {
            establecerData(item);
            $('#modal_insertar_titulo').html('MODIFICAR ' + item.producto);
            $('#modalInsertar').modal('show');
            $('#btnGuardar').attr('disabled', false);
            $('#btnCancelar').attr('disabled', false);
            $('#btnGuardar').unbind();
            ...
            $('#btnGuardar').on('click', function () {
                ...
                $('#frmInsertar').submit(function (e) {
                    e.preventDefault();
                    modificar(item.idproducto);
                });
            });
        });
        $('#eliminar' + item.idproducto).unbind();
        $('#eliminar' + item.idproducto).on('click', function () {
            $('#btn_mensaje_aceptar').attr('disabled', false);
            $('#btn_mensaje_cancelar').attr('disabled', false);
            $('#modal_titulo_mensaje').html('Advertencia');
            $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.producto + '</b> permanentemente?');
        ...
        $('#estado' + item.idproducto).unbind();
        $('#estado' + item.idproducto).on('click', function () {
            $('#btn_mensaje_aceptar').attr('disabled', false);
            $('#btn_mensaje_cancelar').attr('disabled', false);
            $('#modal_titulo_mensaje').html('Advertencia');
            $('#cuerpo_mensaje').html('¿Está seguro que desea modificar el estado de <b>' + item.producto + '</b> permanentemente?');
            $('#modal_mensaje').modal('show');
            $('#btn_mensaje_aceptar').unbind();
            $('#btn_mensaje_aceptar').html('<span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Modificar');
            $('#btn_mensaje_aceptar').on('click', function () {
                modificar_estado(item.idproducto, Math.abs(item.estado - 1));
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'productoMantenimiento.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(id) {
    ...
    formData["ID"] = id;
    $.ajax({
        url: 'productoMantenimiento.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData),
...
function modificar_estado(id, estado) {
    $.ajax({
        url: 'productoMantenimiento.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(id) {
    $.ajax({
        url: 'productoMantenimiento.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + id + '}',
...
function cargarTipoExistencia() {
    $('#cbTipoExistencia').select2({
        ajax: {
            url: 'tipoExistencia_Mantenimiento.aspx/buscar_estado',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            type: 'POST',
            ...
            processResults: function (data, page) {
                ...
                for (var i = 0; i < datos.length; i++) {
                    datos[i].id = datos[i].idtipo_existencia;
                    datos[i].text = datos[i].tipo_existencia;
                }
                return {
                    results: datos
                };
