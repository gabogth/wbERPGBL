    ...
    $.ajax({
        ...
        if (jsonData.result == 'error') {
            ...
            $.each(jsonData.body, function (index, item) {
                ...
                bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idarea_empresa) + '</td>';
                bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idarea_empresa + '"></span></td>';
                bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idarea_empresa + '"></span></td></tr>';
                $('#tbl_body').append(bodyTable);
                bodyTable = '';
                $('#modificar' + item.idarea_empresa).unbind();
                $('#modificar' + item.idarea_empresa).on('click', function () {
                    establecerData(item);
                    ...
                    $('#btnGuardar').on('click', function () {
                        ...
                        $('#frmInsertar').submit(function (e) {
                            e.preventDefault();
                            modificar(item.idarea_empresa);
                        });
                    });
                });
                $('#eliminar' + item.idarea_empresa).unbind();
                $('#eliminar' + item.idarea_empresa).on('click', function () {
                    $('#btn_mensaje_aceptar').attr('disabled', false);
                    $('#btn_mensaje_cancelar').attr('disabled', false);
                    $('#modal_titulo_mensaje').html('Advertencia');
                    $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.area + '</b> permanentemente?');
...
function modificar(id) {
    var query = 'IDAREA_EMPRESA=' + id +
        '&AREA="' + encodeURIComponent($('#txtArea').val()) + '"';

    $.ajax({
        url: 'areaempresa_frmMantenimientoAreaEmpresa.aspx/modificar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function insertar() {
    var query = 'AREA="' + encodeURIComponent($('#txtArea').val()) + '"';

    $.ajax({
        url: 'areaempresa_frmMantenimientoAreaEmpresa.aspx/insertar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function eliminar(id) {
    var query = 'IDAREA_EMPRESA=' + id;
    $.ajax({
        url: 'areaempresa_frmMantenimientoAreaEmpresa.aspx/eliminar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
