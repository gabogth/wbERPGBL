        ...
        if (jsonData.result === 'error') {
            ...
            if (jsonData.body !== null) {
                $.each(jsonData.body, function (index, item) {
                    ...
                    bodyTable += '<td class="text-right">' + ((item.MONEDA_LOCAL !== 1) ? item.VENTA_CABECERA_monto_total_CONVERTIDO.format(2, 3, ',', '.') : '-') + '</td>';
                    bodyTable += '<td class="text-right">' + ((item.MONEDA_LOCAL !== 1) ? item.VENTA_CABECERA_impuesto_CONVERTIDO.format(2, 3, ',', '.') : item.igv.format(2, 3, ',', '.')) + '</td>';
                    bodyTable += '<td class="text-right">' + (item.DESGLOSE_VENTAS_monto_total != null ? item.DESGLOSE_VENTAS_monto_total.format(2, 3, ',', '.') : '0.00') + '</td>';
                    bodyTable += '<td class="text-right">' + (item.DESGLOSE_VENTAS_diferencia != null ? item.DESGLOSE_VENTAS_diferencia.format(2, 3, ',', '.') : (-1 * item.VENTA_CABECERA_monto_total_CONVERTIDO).format(2, 3, ',', '.')) + '</td>';
                    ...
                    $('#btnAccounting' + item.idventas_cabecera).on('click', function () {
                        document.location.href = 'asignarCuentaContable.aspx?ID=' + item.idventas_cabecera;
                    });
