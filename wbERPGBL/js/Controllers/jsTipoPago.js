...
function buscar(indexPag, cantidad) {
    ...
    $.ajax({
        url: 'tipoPago_frmMantenimientoTipoPago.aspx/buscar',
        type: "GET",
        dataType: 'json',
        ...
        if (jsonData.result == 'error') {
            ...
            if (jsonData.body != null) {
                $.each(jsonData.body, function (index, item) {
                    ...
                    bodyTable = '';
                    $('#modificar' + item.idtipo_pago).unbind();
                    $('#modificar' + item.idtipo_pago).on('click', function () {
                        establecerData(item);
                        ...
                        $('#btnGuardar').on('click', function () {
                            ...
                            $('#frmInsertar').submit(function (e) {
                                e.preventDefault();
                                modificar(item.idtipo_pago);
                            });
                        });
                    });
                    $('#eliminar' + item.idtipo_pago).unbind();
                    $('#eliminar' + item.idtipo_pago).on('click', function () {
                        $('#btn_mensaje_aceptar').attr('disabled', false);
                        $('#btn_mensaje_cancelar').attr('disabled', false);
                        $('#modal_titulo_mensaje').html('Advertencia');
                        $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.tipo_pago + '</b> permanentemente?');
                        ...
                        $('#btn_mensaje_aceptar').on('click', function () {
                            eliminar(item.idtipo_pago);
                        });
                    });
                    $('#estado' + item.idtipo_pago).unbind();
                    $('#estado' + item.idtipo_pago).on('click', function () {
                        $('#btn_mensaje_aceptar').attr('disabled', false);
                        $('#btn_mensaje_cancelar').attr('disabled', false);
                        $('#modal_titulo_mensaje').html('Advertencia');
                        $('#cuerpo_mensaje').html('¿Está seguro que desea cambiar el estado de <b>' + item.tipo_pago + '</b>?');
                        $('#modal_mensaje').modal('show');
                        $('#btn_mensaje_aceptar').unbind();
                        $('#btn_mensaje_aceptar').html('<span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Modificar');
                        $('#btn_mensaje_aceptar').on('click', function () {
                            modificar_estado(item.idtipo_pago, Math.abs(item.estado - 1));
                        });
                    });
...
function modificar(id) {
    ...
    $.ajax({
        url: 'tipoPago_frmMantenimientoTipoPago.aspx/modificar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function insertar() {
    ...
    $.ajax({
        url: 'tipoPago_frmMantenimientoTipoPago.aspx/insertar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function eliminar(id) {
    var query = 'IDTIPO_PAGO=' + id;
    $.ajax({
        url: 'tipoPago_frmMantenimientoTipoPago.aspx/eliminar',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
...
function modificar_estado(id, estado) {
    var query = 'IDTIPO_PAGO=' + id +
       '&ESTADO=' + estado;;
    $.ajax({
        url: 'tipoPago_frmMantenimientoTipoPago.aspx/modificar_estado',
        type: "GET",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: query,
