...
function establecerData(data) {
    $('#txtTipoExistencia').val(data.tipo_existencia);
...
function buscar(indexPag, cantidad) {
    $.ajax({
        url: 'tipoExistencia_Mantenimiento.aspx/buscar',
        type: "POST",
        dataType: 'json',
...
function showTableHTML(data, indexPag, cantidad) {
    ...
    $.each(data, function (index, item) {
        ...
        bodyTable += '<td class="text-center">' + getStateIcon(item.estado, 'estado' + item.idtipo_existencia) + '</td>';
        bodyTable += '<td><span class="fa fa-pencil text-warning" style="cursor: pointer;" id="modificar' + item.idtipo_existencia + '"></span></td>';
        bodyTable += '<td><span class="fa fa-trash-o" style="color:white; cursor: pointer;" id="eliminar' + item.idtipo_existencia + '"></span></td></tr>';
        $('#tbl_body').append(bodyTable);
        bodyTable = '';
        $('#modificar' + item.idtipo_existencia).unbind();
        $('#modificar' + item.idtipo_existencia).on('click', function () {
            establecerData(item);
            $('#modal_insertar_titulo').html('MODIFICAR ' + item.tipo_existencia);
            $('#modalInsertar').modal('show');
            $('#btnGuardar').attr('disabled', false);
            $('#btnCancelar').attr('disabled', false);
            $('#btnGuardar').unbind();
            ...
            $('#btnGuardar').on('click', function () {
                ...
                $('#frmInsertar').submit(function (e) {
                    e.preventDefault();
                    modificar(item.idtipo_existencia);
                });
            });
        });
        $('#eliminar' + item.idtipo_existencia).unbind();
        $('#eliminar' + item.idtipo_existencia).on('click', function () {
            $('#btn_mensaje_aceptar').attr('disabled', false);
            $('#btn_mensaje_cancelar').attr('disabled', false);
            $('#modal_titulo_mensaje').html('Advertencia');
            $('#cuerpo_mensaje').html('¿Está seguro que desea eliminar <b>' + item.tipo_existencia + '</b> permanentemente?');
            ...
            $('#btn_mensaje_aceptar').on('click', function () {
                eliminar(item.idtipo_existencia);
            });
        });
        $('#estado' + item.idtipo_existencia).unbind();
        $('#estado' + item.idtipo_existencia).on('click', function () {
            $('#btn_mensaje_aceptar').attr('disabled', false);
            $('#btn_mensaje_cancelar').attr('disabled', false);
            $('#modal_titulo_mensaje').html('Advertencia');
            $('#cuerpo_mensaje').html('¿Está seguro que desea modificar el estado de <b>' + item.tipo_existencia + '</b> permanentemente?');
            $('#modal_mensaje').modal('show');
            $('#btn_mensaje_aceptar').unbind();
            $('#btn_mensaje_aceptar').html('<span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Modificar');
            $('#btn_mensaje_aceptar').on('click', function () {
                modificar_estado(item.idtipo_existencia, Math.abs(item.estado - 1));
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'tipoExistencia_Mantenimiento.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(id) {
    ...
    formData["ID"] = id;
    $.ajax({
        url: 'tipoExistencia_Mantenimiento.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData),
...
function modificar_estado(id, estado) {
    $.ajax({
        url: 'tipoExistencia_Mantenimiento.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(id) {
    $.ajax({
        url: 'tipoExistencia_Mantenimiento.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + id + '}',
