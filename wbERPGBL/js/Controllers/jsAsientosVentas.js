...
function buscar(indexPag, cantidad) {
    ...
    $.ajax({
        url: 'asientoVentas_frmAsientoVentas.aspx/buscar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify({
        ...
        if (jsonData.result === 'error') {
            ...
            if (jsonData.body !== null) {
                $.each(jsonData.body, function (index, item) {
                    ...
                    bodyTable += '<td class="text-right">' + item.VENTA_DETALLE_base_imponible.format(2, 3, ',', '.') + '</td>';
                    bodyTable += '<td class="text-right">' + ((item.MONEDA_LOCAL !== 1) ? item.TIPO_CAMBIO_venta.format(2, 3, ',', '.') : '-') + '</td>';
                    bodyTable += '<td class="text-right">' + ((item.MONEDA_LOCAL !== 1) ? item.VENTA_DETALLE_base_imponible_CONVERTIDO.format(2, 3, ',', '.') : '-') + '</td>';
                    bodyTable += '<td class="text-right">' + ((item.MONEDA_LOCAL !== 1) ? item.VENTA_DETALLE_impuesto_CONVERTIDO.format(2, 3, ',', '.') : item.VENTA_DETALLE_impuesto.format(2, 3, ',', '.')) + '</td>';
                    bodyTable += '<td class="text-center"><span class="fa fa-print fa-lg" style="color:white; cursor: pointer;" id="btnPreview' + item.VENTA_DETALLE_idventas_detalle + '"></span></td>';
                    bodyTable += '<td class="text-center"><span class="fa fa-wpforms fa-lg" style="color:white; cursor: pointer;" id="btnAccounting' + item.VENTA_DETALLE_idventas_detalle + '"></span></td></tr>';
                    $('#tbl_body').append(bodyTable);
                    bodyTable = '';
                    $('#btnAccounting' + item.VENTA_DETALLE_idventas_detalle).unbind();
                    $('#btnAccounting' + item.VENTA_DETALLE_idventas_detalle).on('click', function () {
                        establecerData(item);
                        $('#modal_insertar_titulo').html('FACTURA ' + item.EMPRESA_ALIAS + '-' + item.SERIE_SERIE + '-' + item.correlativo);
                        $('#modalInsertar').modal('show');
                        $('#btnGuardar').attr('disabled', false);
                        $('#btnCancelar').attr('disabled', false);
                        $('#btnGuardar').unbind();
                        $('#btnGuardar').html('<span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;CARGAR');
                        $('#btnGuardar').on('click', function () {
                            $('#frmInsertar').unbind();
                            $('#frmInsertar').submit(function (e) {
                                insertar(item.VENTA_DETALLE_idventas_detalle);
                                $('#txtGlosa').val('');
                                e.preventDefault();
                            });
                        });
                    });
                    $('#btnPreview' + item.VENTA_DETALLE_idventas_detalle).unbind();
                    $('#btnPreview' + item.VENTA_DETALLE_idventas_detalle).on('click', function (e) {
